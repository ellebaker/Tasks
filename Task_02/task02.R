...
varcMass<-tapply(cyrus3$value[cMass], cyrus3$age[Mass], var)
totalcMass<- tapply(cyrus3$valuec[Mass],cyrus3$agec[Mass], sum)
...
plot(as.numeric(names(totalMass)), totalMass, type="b", pch=16, xlab="age in days", ylab="mass in kg")
abline(h=mean(totalMass), lty=2, col="red")
points(cyrus3$value[cMass]~cyrus3$age[cMass], beren3$value[Mass]~cyrus3$age[Mass])
